...
int n,q,p[N + 2],t[N + 2],d1[N + 2],d2[N + 2];
char s[N + 2],c[N * 2 + 2];

bool palind(int l,int r) {
	if(r%2==l%2) return d1[(l+r)/2]>(r-l)/2;
	return d2[(l+r+1)/2]>(r-l)/2;
}

void _main() {
	scanf("%d%d%s",&n,&q,s+1);
	rep(i,1,n)
		t[i]=t[i-1]+(s[i-1]!=s[i+1]||s[i-1]==s[i]);
	rep(i,1,n)
	...
	while(q--) {
		...
		ans=(ans+1)*ans;
		if(t[r-1]!=t[l]) {
			ll x=(r-l+1)/2,y=x*(x+1)/2;
			ans+=y*2-x-1;
		}
		if(!palind(l,r)) ans+=r-l+1;
		printf("%lld\n",ans);
	}
}
