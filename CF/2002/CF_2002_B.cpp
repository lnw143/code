...
int n,a[N + 2],b[N + 2],p[N + 2];

bool solve(int l,int r,int x,int y) {
	if(l>=r) return false;
	if(r-l==y-x) {
		if(a[l]==b[x]||a[l]==b[y]) return solve(l,r-1,x,y);
		else return solve(l+1,r,x,y);
	} else {
		if(p[b[x]]<l||p[b[x]]>r) return solve(l,r,x+1,y);
		if(p[b[y]]<l||p[b[y]]>r) return solve(l,r,x,y-1);
		return true;
	}
}
