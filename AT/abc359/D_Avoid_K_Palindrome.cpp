...
const int N = 1e3, K = 10, S = 1 << K, P = 998244353;
...
bool check(const int sta) {
	if(bz[sta]) return g[sta];
	bz[sta]=true;
	fo(i,0,k)
		if(((sta>>i)&1)!=((sta>>(k-i-1))&1)) {
			g[sta]=true;
			break;
		}
	return g[sta];
...
void _main() {
	...
	fo(i,1,n)
		fu(j,0,m) 
			for(int h=0; h<2; ++h) {
				if(s[i]!='?'&&h!=s[i]-'A') continue;
				int sta=((j<<1)|h)&(m-1);
				if(i<k||check(sta)) (f[i][sta]+=f[i-1][j])%=P;
			}
	ll ans=0;
	fu(i,0,m) (ans+=f[n][i])%=P;
	printf("%lld\n",ans);
}
