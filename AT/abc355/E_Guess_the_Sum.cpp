...
int prepare(int l,int r) {
	if(l>=r||r>(1<<n)) return inf<int>;
	if(l+x==r||l+y==r) return 1;
	const auto u = mkp(l,r);
	if(f.count(u)) return f[u];
	const int x=l>0?(l&-l):inf<int>,y=r&-r;
	auto res=min({
		mkp(x<=y?prepare(l-x,r)+1:inf<int>,0),
		mkp(x<=y?prepare(l+x,r)+1:inf<int>,1),
		mkp(y<=x?prepare(l,r+y)+1:inf<int>,2),
		mkp(y<=x?prepare(l,r-y)+1:inf<int>,3)
	});
	if(res.first>=inf<int>) return inf<int>;
	mp[u]=res.second;
	f[u]=res.first;
	return res.first;
}
...
int call(int l,int r) {
	int x=l&-l,y=r&-r;
	if(l+x==r||l+y==r) return query(l,r);
	switch(mp[mkp(l,r)]) {
		case 0: return call(l-x,r)-call(l-x,l);
		case 1: return call(l,l+x)+call(l+x,r);
		case 2: return call(l,r+y)-call(r,r+y);
		case 3: return call(l,r-y)+call(r-y,r);
		default: error("No!");
	}
}
