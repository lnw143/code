...
const int N = 100, K = 3, M = 1500, inf = 0x3f3f3f3f;
...
void _main() {
	...
	scanf("%d",&n);
	int sum=0;
	fo(i,1,n) scanf("%d%d",&a[i],&b[i]),sum+=b[i];
	...
	fo(i,1,n)
		fo(j,0,sum)
			fo(k,0,sum)
				if(f[i-1][j][k]<inf) {
					if(a[i]==1) f[i][j+b[i]][k]=min(f[i][j+b[i]][k],f[i-1][j][k]);
					else if(a[i]==2) f[i][j][k+b[i]]=min(f[i][j][k+b[i]],f[i-1][j][k]);
					else f[i][j][k]=min(f[i][j][k],f[i-1][j][k]);
					f[i][j+b[i]][k]=min(f[i][j+b[i]][k],f[i-1][j][k]+1);
					f[i][j][k+b[i]]=min(f[i][j][k+b[i]],f[i-1][j][k]+1);
					f[i][j][k]=min(f[i][j][k],f[i-1][j][k]+1);
				}
	int ans=f[n][sum/3][sum/3];
	printf("%d\n",ans<inf?ans:-1);
}
