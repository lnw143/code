...
void _main() {
	scanf("%d%s",&n,s+1);
	fo(i,1,n) {
		f[i][S]=s[i]!='R'?max(f[i-1][R],f[i-1][P])+(s[i]=='P'):-1e9;
		f[i][R]=s[i]!='P'?max(f[i-1][S],f[i-1][P])+(s[i]=='S'):-1e9;
		f[i][P]=s[i]!='S'?max(f[i-1][R],f[i-1][S])+(s[i]=='R'):-1e9;
	}
	printf("%d",max({f[n][R],f[n][S],f[n][P]}));
}
