...
set<int> l[N + 2],r[N + 2],u[N + 2],d[N + 2];
...
void _main() {
	...
	fo(i,1,q) {
		...
		for(auto it=l[x].upper_bound(-p); it!=l[x].end(); it=l[x].upper_bound(-p))
			if(bz[-*it])
				l[x].erase(it);
			else {
				bz[-*it]=true;
		...
		for(auto it=u[y].upper_bound(-p); it!=u[y].end(); it=u[y].upper_bound(-p))
			if(bz[-*it])
				u[y].erase(it);
			else {
				bz[-*it]=true;
		...
		for(auto it=r[x].upper_bound(p); it!=r[x].end(); it=r[x].upper_bound(p))
			if(bz[*it])
				r[x].erase(it);
			else {
				bz[*it]=true;
		...
		for(auto it=d[y].upper_bound(p); it!=d[y].end(); it=d[y].upper_bound(p))
			if(bz[*it])
				d[y].erase(it);
			else {
				bz[*it]=true;
