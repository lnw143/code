...
int n,q,a[N + 2],b[N + 2],sum[N + 2];

namespace segtree {
	const int M = N << 2;
	int sa[M],sb[M],ta[M],tb[M],v[M],s[M];
#define mid (l+r>>1)
#define ls (u<<1)
#define rs (ls|1)
#define li ls,l,mid
#define ri rs,mid+1,r
	...
	void modifyA(int u,int l,int r,int x,int y,int f) {
		if(r<x||y<l) return ;
		if(x<=l&&r<=y) {
			(ta[u]+=f)%=P;
			(val[u]+=f*(sb[u]+tb[u]*(r-l+1ll)%P)%P)%=P;
			return ;
		}
		modifyA(li,x,y,f);
		modifyA(ri,x,y,f);
		val[u]=(val[ls]+val[rs])%P;
	}
