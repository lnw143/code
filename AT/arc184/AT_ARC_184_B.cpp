...
ll count(ll m) {
	static ll f[N * 2 + 2];
	const auto pos = [&](ll x) { return x>N?N+m/x:x; };
	for(ll i=1; i<=m; i=m/(m/i)+1)
		f[pos(m/i)]=m/i-1;
	ll pre=1;
	for(auto i : p) {
		for(ll j=1; j<=m; j=m/(m/j)+1) {
			ll x=m/j;
			if(x>=i*i) f[pos(x)]=f[pos(x)]-f[pos(x/i)]+f[pre];
		}
		pre=i;
		if(i*i>=m) break;
	}
...
void _main() {
	scanf("%d",&n);
	ans=log2l(n);
	ll cnt=count(n),cnt2=cnt;
	for(auto i : p) if(i<=n) cnt2+=n/i-1;
	ans+=cnt-2;
	// ans+=cnt2;
	fo(i,1,n) {
		map<int,int> mp;
		...
		if(mp.size()==2) {
			...
			auto [x,y]=*mp.begin();
			auto [s,t]=*mp.rbegin();
			if(x>3||mp[3]>1&&mp[2]==1||mp[2]>1&&s!=3) ++ans,debug("%d\n",i);
		} else if(mp.begin()->first>3&&mp.begin()->second>1) ++ans,debug("%d\n",i);
	}
	printf("%d",ans);
}
