...
const int N = 3e2, P = 998244353;

int n,p[N + 2],s[N + 2][N + 2];
ll f[N + 2];

int rsum(int x,int y,int l,int r) {
	return s[y][r]-s[x][r]-s[y][l]+s[x][l];
}

bool check(int x,int y) {
	fo(i,x+1,y-1)
		if(p[y]<p[i]&&p[i]<p[x])
			if(rsum(x,i-1,p[y],p[i]-1)+rsum(i,y-1,p[i],p[x]-1)==0)
				return false;
	return true;
}
