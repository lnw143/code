...
void _main() {
	scanf("%d%d%lld",&n,&m,&ans[1]);
	fo(i,1,m) scanf("%d%d%d%d",&e[i].u,&e[i].v,&e[i].s,&e[i].t),p[i]=i;
	sort(p+1,p+m+1,[](int x,int y) {
		return e[x].s<e[y].s;
	});
	st[e[1].v].emplace(e[1].t,e[1].t+ans[1]);
	fo(i,1,m) {
		if(p[i]==1) continue;
		auto [u,v,s,t]=e[p[i]];
		while(st[u].size()&&st[u].begin()->first<=s) {
			maxn[u]=max(maxn[u],st[u].begin()->second);
			st[u].erase(st[u].begin());
		}
		ans[p[i]]=max(0ll,maxn[u]-s);
		st[v].emplace(e[p[i]].t,e[p[i]].t+ans[p[i]]);
	}
	fo(i,2,m) printf("%lld ",ans[i]);
}
