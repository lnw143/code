...
int make(const vec<int>& f) {
	int u=0;
	fd(i,n+1,0) u=u*3+f[i];
	return u;
}
...
int bfs() {
	...
	while(Q.size()) {
		...
		{
			int t=u;
			fo(i,1,n+2) p.pbk(t%3),t/=3;
		}
		int v=0;
		fo(i,0,n+1) if(p[i]==0) v=i;
		--v;
		fo(x,0,n) {
			if(abs(x-v)<=1) continue;
			swap(p[x],p[v]);
			swap(p[x+1],p[v+1]);
			int e=make(p);
			if(!vis[e]) vis[e]=true,f[e]=f[u]+1,Q.push(e);
			swap(p[x],p[v]);
			swap(p[x+1],p[v+1]);
		}
	}
