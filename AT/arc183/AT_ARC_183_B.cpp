...
void _main() {
	...
	if(k==1) {
		vector<int> s;
		fo(i,1,n) if(i==1||a[i]!=a[i-1]) s.push_back(a[i]);
		int j=0;
		fo(i,1,n)
			for(; j<s.size()&&s[j]!=b[i]; ++j);
		Yes(j<s.size());
		return ;
	}
	bool ok=true;
	...
	if(ok) return Yes();
	set<int> st;
	fo(i,1,n) st.insert(a[i]);
	fo(i,1,n) if(st.count(b[i])==0) return No();
	fo(i,1,n) lst[i]=0;
	fo(i,1,n) {
		if(lst[b[i]]&&i-lst[b[i]]<=k) return Yes();
		lst[b[i]]=i;
	}
	No();
}
