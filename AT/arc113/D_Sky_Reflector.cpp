...
void _main() {
	scanf("%d%d%d",&n,&m,&k);
	if(n==1) return printf("%lld",qpow(k,m,P)),void();
	if(m==1) return printf("%lld",qpow(k,n,P)),void();
	if(k==1) return printf("1"),void();
	mint ans=0;
	for(int i=1; i<=k; ++i)
		ans+=(qpow(k-i+1,m,P)-qpow(k-i,m,P))*qpow(i,n,P)%P;
	printf("%d",ans.safe_mod());
}
