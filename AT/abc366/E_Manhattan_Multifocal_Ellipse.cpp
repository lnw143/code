...
const int N = 2e5, D = 1e6, M = D * 4;

int n,d,x[N + 2],y[N + 2];

int px[M + 2],py[M + 2],sum[D + 2];

ll dx[M + 2],dy[M + 2];

void solve(int *p,ll *d) {
	fo(i,1,M) d[0]+=p[i]*(ll)i;
	fo(i,1,M) p[i]+=p[i-1];
	fo(i,1,M)
		d[i]=d[i-1]+p[i-1]-(n-p[i-1]);
}
