...
int n,a[N + 2][N + 2][N + 2],q;

void _main() {
	scanf("%d",&n);
	fo(i,1,n) fo(j,1,n) fo(k,1,n) scanf("%d",&a[i][j][k]);
	fo(i,1,n) fo(j,1,n) fo(k,1,n) a[i][j][k]+=a[i-1][j][k];
	fo(i,1,n) fo(j,1,n) fo(k,1,n) a[i][j][k]+=a[i][j-1][k];
	fo(i,1,n) fo(j,1,n) fo(k,1,n) a[i][j][k]+=a[i][j][k-1];
	scanf("%d",&q);
	fo(i,1,q) {
		int xl,xr,yl,yr,zl,zr;
		scanf("%d%d%d%d%d%d",&xl,&xr,&yl,&yr,&zl,&zr);
		--xl,--yl,--zl;
		printf("%d\n",a[xr][yr][zr]
			-a[xl][yr][zr]
			-a[xr][yl][zr]
