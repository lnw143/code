...
struct hash {
	llu val[N + 2],suma[N + 2],sumb[N + 2];
	void init() {
		fo(i,1,n) val[i]=rnd();
		fo(i,1,n) suma[i]=suma[i-1]+val[a[i]];
		fo(i,1,n) sumb[i]=sumb[i-1]+val[b[i]];
	}
	bool check(int l,int r,int x,int y) {
		return suma[r]-suma[l-1]==sumb[y]-sumb[x-1];
	}
} h[K + 2];
