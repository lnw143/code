...
const int N = 50, S = N * N;

int n,m,t,sx,sy;

ll a[N + 2][N + 2],f[N + 2][N + 2][S + 2];

void _main() {
	scanf("%d%d%d%d%d",&n,&m,&t,&sx,&sy);
	...
	fo(i,1,min(n*m-1,t)) {
		fo(j,1,n)
			fo(k,1,m)
				if(f[j][k][i-1]>=0) {
					f[j][k][i]=max(f[j][k][i],f[j][k][i-1]+a[j][k]);
					for(auto [x,y] : way4) {
						int j_=j+x,k_=k+y;
						if(j_<1||n<j_||k_<1||m<k_) continue;
						f[j_][k_][i]=max(f[j_][k_][i],f[j][k][i-1]+a[j_][k_]);
					}
				}
		fo(j,1,n)
			fo(k,1,m)
				if(f[j][k][i]>=0)
					ans=max(ans,f[j][k][i]+(t-i)*a[j][k]);
	}
	printf("%lld\n",ans);
}
