...
const int N = 1e2, S = N * N;

int n,m,k;

bool rig[N + 2][N + 2],dow[N + 2][N + 2];

void _main() {
	scanf("%d%d%d",&n,&m,&k);
	if((n&1)!=(k&1)||k<n) return No();
	Yes();
	int s=(k-n)/2;
	for(int i=1; i<n; i+=2) {
		int e=m;
		for(int j=m-1; j>=1&&s>0; --j) {
			--s;
			rig[i][j]=rig[i+1][j]=true;
			e=j;
		}
		dow[i][e]=true;
		dow[i+1][m]=true;
	...
	dow[n][m]=true;
	if(n&1) fo(i,1,s) {
		dow[n-1][i*2-1]=dow[n-1][i*2]=true;
		rig[n-1][i*2-1]=false;
		rig[n][i*2-1]=true;
	}
	fo(i,1,2*m+1) printf("%c","+S"[i==2*m]); putchar('\n');
	fo(i,1,n) {
		printf("+");
		for(int j=1; j<m; ++j) printf("o%c","|."[rig[i][j]]);
		printf("o+\n");
		if(i<n) {
			for(int j=1; j<=m; ++j) printf("+%c","-."[dow[i][j]]);
			printf("+\n");
		}
	}
	fo(i,1,2*m+1) printf("%c","+G"[i==2*m]); putchar('\n');
}
