...
const int N = 1e3, Sig = 26, P = 998244353;

int n,a[Sig + 2];
ll f[Sig + 2][N + 2],fac[N + 2],inv[N + 2],ifac[N + 2];
...
void _main() {
	...
	fu(i,0,Sig) scanf("%d",&a[i]);
	fo(i,0,a[0]) f[0][i]=1;
	fu(i,1,Sig)
		fo(j,0,n)
			fo(k,0,min(a[i],n-j))
				(f[i][j+k]+=f[i-1][j]*binom(j+k,k)%P)%=P;
	ll ans=0;
	fo(i,1,n) (ans+=f[Sig-1][i])%=P;
	printf("%lld",ans);
}
