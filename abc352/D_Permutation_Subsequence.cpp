...
int n,k,p,a[N + 2],ans=inf<int>;

deque<pair<int,int>> mx,mn;

void put(int i,int x) {
	while(!mx.empty()&&mx.back().first<i) mx.pop_back();
	mx.ebk(i,x);
	while(!mn.empty()&&mn.back().first>i) mn.pop_back();
	mn.ebk(i,x);
}

int find_max(int i) {
	while(!mx.empty()&&mx.front().second<i) mx.pop_front();
	return mx.front().first;
}

int find_min(int i) {
	while(!mn.empty()&&mn.front().second<i) mn.pop_front();
	return mn.front().first;
}
...
void _main() {
	...
	rep(i,1,k-1) put(a[i],i);
	rep(i,k,n) {
		put(a[i],i);
		umn(ans,find_max(i-k+1)-find_min(i-k+1));
	}
	printf("%d",ans);
}
